      ...
      >
        ...
        <form (ngSubmit)="onCreateServer()">
          ...
          <div class="mb-4">
            <label
              for="channelName"
              class="block text-typo-secondary text-sm font-medium mb-1"
            >
              Название сервера
            </label>
            <input
          ...
          <div class="flex justify-center gap-2 mt-6">
            <button
              type="submit"
              class="px-4 py-2 bg-green-500 text-sm rounded-xl text-white hover:bg-green-600 transition"
            >
              Создать сервер
            </button>
          </div>
  ...
  onCreateServer() {
    ...
    this.apiService.postCreateServer(formData).subscribe({
      next: (response) => {
        ...
        const defaultChannelId = response.default_channel_id;
        this.modalService.close();
        this.router.navigate(['/channels', serverId, defaultChannelId]);
      },
      error: (error) => {
